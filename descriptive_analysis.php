...
function generate_insights($analytics_report, $selected_year) {
    ...
    if (!empty($monthly_trends)) {
        ...
        $q1 = array_sum(array_map(function($m) { return $m['month'] <= 3 ? intval($m['monthly_cases']) : 0; }, $monthly_trends));
        $q2 = array_sum(array_map(function($m) { return $m['month'] > 3 && $m['month'] <= 6 ? intval($m['monthly_cases']) : 0; }, $monthly_trends));
        $q3 = array_sum(array_map(function($m) { return $m['month'] > 6 && $m['month'] <= 9 ? intval($m['monthly_cases']) : 0; }, $monthly_trends));
        $q4 = array_sum(array_map(function($m) { return $m['month'] > 9 ? intval($m['monthly_cases']) : 0; }, $monthly_trends));
    ...
    if (!empty($analytics_report['barangay_analysis'])) {
        ...
        $total_in_top_3 = array_sum(array_map(function($b) { return intval($b['count']); }, $top_3_barangays));
        $total_cases = intval($analytics_report['summary']['total_cases']);
        $concentration_rate = ($total_cases > 0) ? ($total_in_top_3 / $total_cases) * 100 : 0;
        
        $insights['geographic'] = [
            'top_barangays' => array_map(function($b) { return $b['barangay']; }, $top_3_barangays),
            'concentration_rate' => number_format($concentration_rate, 1),
            'interpretation' => "Geographic concentration analysis reveals that " . number_format($concentration_rate, 1) . "% of all cases are concentrated in just 3 barangays: " . implode(', ', array_map(function($b) { return $b['barangay']; }, $top_3_barangays)) . ". This geographic clustering suggests specific environmental or socioeconomic factors contributing to higher exposure rates."
        ];
    }
